			...
			try {
				...
				default:
					...
					if (methodName == currentPropertyGetter) {
						...
						if ((returnValue != null) && (returnValue.GetType() != column.Value) && (!SqlDeserializer.IsNullableType(returnValue.GetType())) && typeof(IConvertible).IsAssignableFrom(returnValue.GetType())) {
