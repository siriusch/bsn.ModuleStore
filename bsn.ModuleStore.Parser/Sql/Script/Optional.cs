...
namespace bsn.ModuleStore.Sql.Script {
	public class Optional<T>: SqlToken where T: SqlToken {
		...
		[Rule("<FulltextColumnType> ::= ~TYPE_COLUMN <TypeNameQualified>", typeof(Qualified<SchemaName, TypeName>))]
		...
		[Rule("<OptionalNotForReplication> ::= ~NOT FOR_REPLICATION", typeof(ForReplicationToken))]
		[Rule("<OptionalForeignRefColumn> ::= ~'(' <ColumnName> ~')'", typeof(ColumnName))]
		[Rule("<CTEGroup> ::= ~WITH <CTEList>", typeof(Sequence<CommonTableExpression>))]
		...
		[Rule("<OptionalAlias> ::= ~<OptionalAs> <AliasName>", typeof(AliasName))]
