...
namespace bsn.ModuleStore.Sql.Script {
	public sealed class DisableTriggerStatement: EnableDisableTriggerStatement {
		[Rule("<DisableTriggerStatement> ::= ~DISABLE ~TRIGGER ~ALL ~ON <TriggerTarget>")]
		public DisableTriggerStatement(TriggerTarget target): this(null, target) {}

		[Rule("<DisableTriggerStatement> ::= ~DISABLE ~TRIGGER <TriggerNameQualifiedList> ~ON <TriggerTarget>")]
